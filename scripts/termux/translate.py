...
upgrade. A chunk that does not come back with exactly one line per input is redone a line at a time.
...
def ask(lines):
    ...
    out = subprocess.run(
        [LLAMA, '-m', MODEL, '-c', '2048', '--no-repack', '-p', prompt, '-n', str(40 * len(lines) + 40), '--temp', '0.2',
