...
- [ ] T001 Initialise a SvelteKit + TypeScript project at the repository root, keeping existing `docs/`, `specs/` and `.specify/` untouched
- [ ] T002 Install and configure `@sveltejs/adapter-static` in `svelte.config.js`, setting `paths.base` for a GitHub Pages subpath
- [ ] T003 [P] Configure `vitest` and `fast-check` in `vite.config.ts` and `package.json`
- [ ] T004 [P] Configure ESLint and Prettier in `eslint.config.js` and `.prettierrc`
- [ ] T005 [P] Install `@sqlite.org/sqlite-wasm` and add the required COOP/COEP-free OPFS setup notes to `README.md`
- [ ] T006 Create the empty directory skeleton `src/lib/{domain,analyzer,content,storage,diagnostics,ui}/` and `tests/{domain,architecture,storage}/` per plan.md
...
- [ ] T009 [P] Write failing test in `tests/storage/migration.test.ts` asserting the schema contains `provenance`, `user_id`, `device_id`, `device_seq`, `document_id`, `from_offset`, `to_offset` and `observed_pronunciation`
...
- [ ] T011 [P] Define domain types in `src/lib/domain/types.ts` — `Lexeme`, `Token`, `WordState`, `HistoryEntry`, `Occurrence` — per data-model.md
- [ ] T012 Write `src/lib/storage/migrations/001-initial.sql` creating `lexeme`, `document`, `token`, `word_state`, `status_event`, `device` and `diagnostic` per data-model.md, with every hedge column present
...
- [ ] T016 [P] [US1] Define the `Analyzer` interface in `src/lib/analyzer/types.ts` per contracts/analyzer.md — `analyze` returns a `Promise`, deliberately, so a later async and fallible analyzer fits
- [ ] T017 [P] [US1] Define the `ContentSource` interface in `src/lib/content/types.ts` per contracts/content-source.md
- [ ] T018 [US1] Implement the placeholder analyzer in `src/lib/analyzer/character.ts` — one token per code point, `name: "character-splitter"`, `version: "1"`, `isWord` true for CJK ranges. **Do not improve it.**
- [ ] T019 [US1] Implement `src/lib/content/paste.ts` — validate non-empty and within ~5,000 code points, emit `text/plain` with a title from the opening characters (FR-018, FR-020)
- [ ] T020 [US1] Implement `saveDocument`, `listDocuments` and `getDocument` in `src/lib/storage/repository.ts`; `getDocument` returns retained raw content **and** tokens, per contracts/repository.md
- [ ] T021 [US1] Build the library screen in `src/routes/+page.svelte` — list saved documents, paste-and-save form, rejection messages
- [ ] T022 [US1] Build the reader screen in `src/routes/read/[id]/+page.svelte` — render tokens as discrete elements, mobile-first, no horizontal scroll (FR-017)
...
- [ ] T023 [P] [US2] Write failing tests in `tests/domain/state.test.ts` for the state set and the projection — that current state is a function of history alone, using `fast-check`'s model-based testing over generated assertion sequences
- [ ] T024 [P] [US2] Write failing test in `tests/domain/history.test.ts` asserting that folding entries in `(device_id, device_seq)` order reproduces `word_state` exactly for every lexeme (FR-011)

### Implementation

- [ ] T025 [P] [US2] Implement `src/lib/domain/state.ts` — the extensible state set as data, and slice 0's trivial projection (latest assertion wins) per FR-006a and FR-010b
- [ ] T026 [US2] Implement `src/lib/domain/history.ts` — append-only entry construction recording *what was asserted*, never what the state became (FR-010a)
- [ ] T027 [US2] Implement `assertState` in `src/lib/storage/repository.ts` — append `status_event` first, allocating `device_seq`, then update `word_state`; never the reverse
- [ ] T028 [US2] Implement `getStates` and `rebuildProjection` in `src/lib/storage/repository.ts`; `rebuildProjection` must be exercised by T024, not merely written
- [ ] T029 [US2] Implement find-or-create of lexemes in `src/lib/storage/repository.ts`, applying the language provider's identity rule rather than the repository's own (FR-009)
- [ ] T030 [US2] Build the state menu in `src/lib/ui/StateMenu.svelte` — opens on tap, lists available states from configuration, adequate tap targets
- [ ] T031 [US2] Wire tapping and visual state distinction into `src/routes/read/[id]/+page.svelte`; untouched words must be visually distinct from any chosen state (FR-006b)
...
- [ ] T032 [P] [US3] Implement the on-device diagnostics record in `src/lib/diagnostics/log.ts`, writing to the `diagnostic` table (FR-021)
- [ ] T033 [US3] Surface failures in the interface via `src/lib/ui/ErrorNotice.svelte`, distinguishing a refused input, a storage failure and an unexpected error (FR-022)
...
**Within US1**: T015–T017 are parallel. T018 needs T016; T019 needs T017; T020 needs both; T021 and
T022 need T020.

**Within US2**: T023 and T024 are parallel. T025 blocks T026, which blocks T027–T029. T030 and T031
follow.
...
```
T016 src/lib/analyzer/types.ts
T017 src/lib/content/types.ts
```

**Phase 6** — T038 through T041 are independent checks.
...
**Do not improve the placeholder analyzer (T018).** A better character splitter makes the slice
