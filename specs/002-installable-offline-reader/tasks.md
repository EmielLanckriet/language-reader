...
- [ ] T008 [US1] Write `src/service-worker.ts` `install` handler: open `cache-${version}` from `$service-worker` and `cache.addAll([...build, ...files])`, letting a failure reject installation so a half-cached version never activates (FR-007)
- [ ] T009 [US1] Add the `activate` handler to `src/service-worker.ts`: delete every cache whose name is not the current one, then `clients.claim()`. Do **not** call `skipWaiting()` in `install` — see contracts/service-worker.md for why its absence is deliberate (FR-009)
