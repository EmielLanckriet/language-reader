...
- [ ] T085 Carry the upgrade in the schema: migration `002-partial-upgrade.sql` adding `upgrade_analyzer`, `upgrade_version` and `upgraded_through`, and `Repository.advanceUpgrade` writing one batch and its boundary in a single transaction — collapsing to a plain stamp when the boundary reaches the end, and refusing a batch whose start is not where the recorded upgrade left off
- [ ] T086 Prove in `tests/storage/partial-upgrade.test.ts` the properties that make the boundary trustworthy rather than plausible: **any sequence of batch boundaries produces exactly the tokens one whole-document pass produces**; every intermediate state still tiles the document; marks survive every batch (FR-023, FR-024); an upgrade by an analyzer no longer in force is discarded rather than resumed; and a document interrupted mid-upgrade re-reads as the prefix analyzer before the boundary and the stamped analyzer after it
- [ ] T087 Pass `advanceUpgrade` across the worker seam in `protocol.ts`, `client.ts` and `worker.ts`, staying out of `READER_CHANGES` for the reason `replaceTokens` already does: this is the application catching up with itself, not a change the reader made
- [ ] T088 Write the batching driver in `src/lib/storage/upgrade.ts`: whole segmentation units, a budget measured in elapsed milliseconds rather than guessed in characters, and **a real task boundary between units** — with `tests/storage/upgrade.test.ts` asserting both that the batch respects unit boundaries and that a timer scheduled before a batch actually runs during it
- [ ] T089 Make the sweep resumable: batch by batch, asking `shouldContinue` between batches rather than only between documents, so losing visibility mid-document costs one batch instead of the whole pass (FR-018, FR-021)
- [ ] T090 Let the reader watch it happen: the open document re-reads its tokens when a batch lands, keeps the reader's marks and menu, and says how far the upgrade has reached rather than showing a stamp that describes only part of what is on screen
