...
- [ ] T081 On the phone: confirm a 4,999-character document now appears within 3 seconds, and say whether the background upgrade to the model is tolerable or whether it makes reading unpleasant while it runs
- [ ] T082 Find out why downloading the model hangs under headless Chrome in `scripts/verify-in-browser`. Network, streamed `cache.put`, service-worker cache contention and the download loop itself are all measured good (research.md R19); the leading untested hypothesis is the per-chunk progress callback writing Svelte state against a throttled headless renderer. Until this is answered, `bigimport` can only be run in its weaker `--no-warm` form
