...
- [ ] T093 Find out why `readonly` times out waiting for the background copy to refuse or accept. **Not a regression from Phase 11**: it fails identically on the commit before it, checked by building that commit in a worktree and running the same scenario. Either slice 1's storage lease has stopped refusing, or the harness cannot see it happen — and the two are not distinguishable from the outside, which is the part that makes it worth answering
- [ ] T094 Account for `bigimport` flakiness before trusting a single run of it: four runs gave 328 ms, 462 ms, one cold-start outlier of 4,516 ms on a freshly built tree, and one failure that produced no measurement at all. The outlier is explainable and the silent failure is not
