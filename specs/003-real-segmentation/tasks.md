...
Reported from the phone once T080 shipped: documents open at once, and none of them ever stops being
dictionary-segmented. Three independent causes, each sufficient on its own (research.md R20). The
third has a design consequence and gets an ADR; the other two are repairs.

- [X] T083 Decide and record in [ADR-0016](../../docs/adr/0016-documents-may-be-mid-upgrade.md) how a partly upgraded document stays honestly stamped: two stamps and a boundary, with the alternatives rejected on their costs
- [X] T084 Record the three causes in research.md R20, and commit the measurement that separates the first from the intuition it contradicts — `scripts/measure/yield.mjs`, showing that a timer does not run during an await-only loop and does during a yielding one
- [X] T085 Carry the upgrade in the schema: migration `002-partial-upgrade.sql` adding `upgrade_analyzer`, `upgrade_version` and `upgraded_through`, and `Repository.advanceUpgrade` writing one batch and its boundary in a single transaction — collapsing to a plain stamp when the boundary reaches the end, and refusing a batch whose start is not where the recorded upgrade left off
- [X] T086 Prove in `tests/storage/partial-upgrade.test.ts` the properties that make the boundary trustworthy rather than plausible: **any sequence of batch boundaries produces exactly the tokens one whole-document pass produces**; every intermediate state still tiles the document; marks survive every batch (FR-023, FR-024); an upgrade by an analyzer no longer in force is discarded rather than resumed; and a document interrupted mid-upgrade re-reads as the prefix analyzer before the boundary and the stamped analyzer after it
- [X] T087 Pass `advanceUpgrade` across the worker seam in `protocol.ts`, `client.ts` and `worker.ts`, staying out of `READER_CHANGES` for the reason `replaceTokens` already does: this is the application catching up with itself, not a change the reader made
- [X] T088 Write the batching driver in `src/lib/storage/upgrade.ts`: whole segmentation units, a budget measured in elapsed milliseconds rather than guessed in characters, and **a real task boundary between units** — with `tests/storage/upgrade.test.ts` asserting both that the batch respects unit boundaries and that a timer scheduled before a batch actually runs during it
- [X] T089 Make the sweep resumable: batch by batch, asking `shouldContinue` between batches rather than only between documents, so losing visibility mid-document costs one batch instead of the whole pass (FR-018, FR-021)
- [X] T090 Let the reader watch it happen: the open document re-reads its tokens when a batch lands, keeps the reader's marks and menu, and says how far the upgrade has reached rather than showing a stamp that describes only part of what is on screen
- [X] T091 Verify in a browser that an interrupted upgrade resumes where it stopped rather than restarting — **not possible as written, and replaced.** The sweep only runs when the analyzer in force differs from the document's stamp, which without the model never happens, and the only way to make it happen headless would be to ship a hook that exists for the harness. What was done instead: the resume path is covered against a real database in `tests/storage/sweep.test.ts`, the risk that actually threatens the reader's phone — applying migration 002 to a database that already holds documents and marks — is covered in `tests/storage/migration.test.ts`, and `shell`, `words`, `offline` and `bigimport --no-warm` were re-run for regressions
- [X] T093 Find out why `readonly` times out waiting for the background copy to refuse or accept — **answered: neither. The application reloaded itself 614 ms into a first visit and the reload landed on the click** (research.md R21). `UpdateOffer` reloaded on any `controllerchange`, including the first install's `clients.claim()`. The lease was never broken; `readonly` now passes and reports the refusal. Fixed, and `firstload` added so it cannot come back unnoticed
- [X] T094 Account for `bigimport` flakiness before trusting a single run of it — **same cause as T093.** The reload was racing the 4,999-character paste and which side won decided what was measured. After the fix: 621, 613, 621 ms across three runs. The checks were not flaky; the application was
- [X] T092 On the phone: import a long document, watch it improve while reading it, lock the phone part-way, and confirm it resumes rather than starting again — **all three confirmed on build 1788513182746**: the percentage shows, the words improve in steps, and it resumes after the phone is locked. One observation: a noticeable wait before the model starts, which the reader accepted. That is the 1,172 ms inference-session creation plus the idle wait, and it is exactly what the register's "pre-warm the inference session at start-up" follow-on addresses
