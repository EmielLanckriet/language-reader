...
| Change | Plausibility | Retrofit cost | Reasoning | Action |
|---|---|---|---|---|
| Hand word lookups to Pleco instead of shipping definitions | medium | cheap | **Raised by the developer 2026-09-03**: Pleco is already on the phone and already trusted for lookups. Slice 2's dictionary was justified partly on slice 3 needing CC-CEDICT's glosses anyway; if lookups are delegated, that justification does not hold and the reader never ships definitions at all — only the 0.43 MB headword list segmentation needs regardless. Pure presentation over derived data either way. Needs checking whether a browser can hand a word to Pleco on Android at all, which is a fact about the app rather than a design choice. | Defer; **check before planning slice 3's dictionary work** |
