...
| Speech-to-text for audio without a transcript | medium | cheap | Content source. Lower priority than it looks: subtitles and YouTube transcripts both rate high and arrive *with* text. If a transcript is produced by STT it is derived and **the audio is the retained input** — keeping only the transcript forfeits re-deriving it with a better model, the same trap as discarding source text. **Built 2026-09-25** ([ADR-0019](adr/0019-transcripts-stream-from-termux.md)): Whisper in Termux, streamed in 30 s chunks so the first lines arrive in seconds; the audio stays kept beside the transcript. Phone speed not yet measured. | Built |
| Backup of earned data outside the app's storage | high | medium | **Built 2026-09-25** ([ADR-0020](adr/0020-earned-data-copied-to-termux.md), spec 005): a versioned snapshot sent to a Termux service within 5 minutes, restored exactly on an empty library. Emulator-verified (a full wipe restores in 9 s); not phone-verified. A manual "back up now" export is the deferred second destination. | Built |
| Translation of each line, offline | high | cheap | **Built 2026-09-25** ([ADR-0021](adr/0021-line-translation-by-a-local-model-in-termux.md)): Qwen3-1.7B in Termux at import, English per line, revealed on tap. Derived. Next: contextual glosses per word from the same model. | Built |
